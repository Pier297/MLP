...
def run_grid_search(conf, train_X=train_X, train_Y=train_Y, val_X=val_X, val_Y=val_Y):
    ...
    K = 3
    for t in range(K):
        model = Sequential(conf, dimension_in, dimension_out)

        if conf["loss_function"] == "MSE":
            loss_func = MSE()

        (train_errors, val_errors, early_best_epoch) = early_stopping(model, loss_func, conf["lr"], conf["l2"], conf["momentum"], conf["BATCH_SIZE"], train_X, train_Y, val_X, val_Y, MAX_UNLUCKY_STEPS=50, MAX_EPOCHS=250)
...
if __name__ == '__main__':
    hyperparameters = generate_hyperparameters(
        ...
        lr_values = [0.2, 0.3, 0.4],
        l2_values = [0],
        momentum_values = [0.2, 0.4, 0.7],
    ...
    model = Sequential(best_hyperparameters, in_dimension=17, out_dimension=1)
    ...
    print("Train accuracy =", accuracy(model, train_X, train_Y))
    print("Validation accuracy =", accuracy(model, val_X, val_Y))
    #print("Test accuracy =", accuracy(model, test_X, test_Y))
