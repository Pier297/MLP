from MLP.Network import Sequential, model_predict
...
if __name__ == '__main__':
    ...
    hyperparameters = generate_hyperparameters(
        ...
        lr                     = [0.1, 0.01, 0.01, 0.05], # 0.6
        l2                     = [0],
        momentum               = [0.3],
        decay_rate_1           = [0.6, 0.7, 0.8, 0.9],
        decay_rate_2           = [0.8, 0.85, 0.99],
    ...
    train_output = model_predict(model, train_input)
    test_output  = model_predict(model, test_input)
    ...
    plot_model_selection_learning_curves(best_results['trials'], name=best_hyperparameters['loss_function_name'], highlight_best=True, file_name='MLP/experiments/results/monk3/model_selection_errors.png')
    plot_model_selection_accuracies(best_results['trials'], highlight_best=True, file_name='MLP/experiments/results/monk3/model_selection_accuracies.png')

    # Plot the final learning curve while training on all the data
    plot_final_training_with_test_error     (final_results['train_errors'],     final_results['watch_errors'],     name=best_hyperparameters['loss_function_name'], file_name='MLP/experiments/results/monk3/final_errors.png')
    plot_final_training_with_test_accuracies(final_results['train_accuracies'], final_results['watch_accuracies'], show=True, file_name='MLP/experiments/results/monk3/final_accuracies.png')
